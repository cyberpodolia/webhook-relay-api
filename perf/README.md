...
- Run the API on the host machine (`uvicorn app.main:app --host 0.0.0.0 --port 8000`).
- Containers use `host.docker.internal:8000` to scrape relay metrics and to drive k6 traffic.
...
- Host-relay target from k6 container: `http://host.docker.internal:8000`
...
- host relay metrics: `host.docker.internal:8000/metrics`
...
```powershell
cd perf
.\scripts\run-k6.ps1 -Scenario baseline.js
```
